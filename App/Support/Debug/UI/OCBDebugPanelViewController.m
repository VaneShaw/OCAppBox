...
- (void)viewDidLoad
{
    ...
    self.environmentButton = [self darkButtonWithTitle:@"切换网络环境" action:@selector(cycleNetworkEnvironment)];
...
- (void)viewDidLayoutSubviews
{
    ...
    self.environmentButton.frame = CGRectMake(24.0, CGRectGetMaxY(self.featureButton.frame) + 12.0, width - 48.0, 48.0);
    CGFloat textViewHeight = CGRectGetHeight(self.view.bounds) - CGRectGetMaxY(self.environmentButton.frame) - 42.0;
    self.snapshotTextView.frame = CGRectMake(24.0,
                                             CGRectGetMaxY(self.environmentButton.frame) + 18.0,
                                             width - 48.0,
...
- (void)reloadSnapshot
{
    ...
    [sections addObject:[self networkSection:network]];
...
- (void)cycleNetworkEnvironment
{
    id<OCBNetworking> network = [self.appContext.serviceRegistry serviceForProtocol:@protocol(OCBNetworking)];
    NSArray<NSString *> *environments = [network registeredEnvironments];
    if (environments.count == 0) {
        return;
    }

    NSString *currentEnvironment = [network currentEnvironment];
    NSUInteger currentIndex = [environments indexOfObject:currentEnvironment];
    NSUInteger nextIndex = currentIndex == NSNotFound ? 0 : ((currentIndex + 1) % environments.count);
    self.appContext.environment = environments[nextIndex];
    [self reloadSnapshot];
}
...
- (NSString *)networkSection:(id<OCBNetworking>)network
{
    NSString *environment = [network currentEnvironment];
    NSURL *baseURL = [network baseURLForEnvironment:environment];
    NSString *baseURLText = baseURL.absoluteString ?: @"-";
    NSArray<NSString *> *registeredEnvironments = [network registeredEnvironments];
    NSString *profiles = registeredEnvironments.count > 0 ? [registeredEnvironments componentsJoinedByString:@", "] : @"-";
    NSString *headerText = @"-";

    if ([network isKindOfClass:[OCBNetworkClient class]]) {
        OCBNetworkClient *client = (OCBNetworkClient *)network;
        if (client.commonHeaders.count > 0) {
            NSMutableArray<NSString *> *pairs = [[NSMutableArray alloc] init];
            [[client.commonHeaders.allKeys sortedArrayUsingSelector:@selector(compare:)] enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx, BOOL *stop) {
                [pairs addObject:[NSString stringWithFormat:@"%@=%@", key, client.commonHeaders[key]]];
            }];
            headerText = [pairs componentsJoinedByString:@", "];
        }
    }
